...
pub fn hud_lines(m: &Model, typed: &str) -> Vec<String> {
    let mut header =
        format!("{TITLE}{BOLD}easyjump{RESET} {DIM}hints are in the sidebar · ` back · esc{RESET}");
    if !typed.is_empty() {
...
pub fn render_list_frame(m: &Model, typed: &str, cols: usize, rows: usize) -> Vec<String> {
    let mut header =
        format!("{TITLE}{BOLD} easyjump {RESET}{DIM}type a hint to jump · ` back · q/esc close{RESET}");
    if !typed.is_empty() {
        header.push_str(&format!("  {HINT_TYPED} {typed} {RESET}"));
    }
