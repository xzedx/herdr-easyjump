...
impl Model {
    pub fn new(snap: Snapshot, ctx: Context, mode: Mode) -> Self {
        ...
        match mode {
            Mode::Sidebar => {
                for (ws, _) in &ws_rows {
                    add(Kind::Workspace, &ws.workspace_id);
                    let mut ws_agents: Vec<&Agent> = agents
                        .values()
                        .filter(|a| a.workspace_id == ws.workspace_id)
                        .collect();
                    ws_agents.sort_by_key(|a| (tab_number(&a.pane_id), a.pane_id.clone()));
                    for a in ws_agents {
                        add(Kind::Pane, &a.pane_id);
                    }
                }
                for p in &tab_panes {
                    add(Kind::Pane, &p.pane_id);
                }
                if current_ws_tabs.len() > 1 {
                    for t in &current_ws_tabs {
                        add(Kind::Tab, &t.tab_id);
                    }
                }
            }
            Mode::List => {
        ...
        let labels = make_labels(dests.len());
    ...
    pub fn publish_hints(&self, client: &mut Client, typed: &str) {
        for (method, key, dest) in self.sidebar_targets() {
            ...
            let mut params = json!({ key: dest.1, "source": SOURCE });
            if label.starts_with(typed) {
                params["tokens"] = json!({ TOKEN: format!("[{label}]") });
                params["ttl_ms"] = json!(TOKEN_TTL_MS);
            } else {
    ...
    /// Clear the token on every workspace and agent pane, labels or not.
    /// Used by the detached `--clear` helper.
    pub fn clear_all(client: &mut Client, snap: &Snapshot) {
        ...
        for a in &snap.agents {
            let _ = client.call(
                ...
                json!({ "pane_id": a.pane_id, "source": SOURCE, "tokens": { TOKEN: Value::Null } }),
            );
        }
    }
